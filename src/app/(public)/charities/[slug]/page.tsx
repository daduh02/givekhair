...
function StatCard({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <div className="surface-card p-6">
      ...
      <p className="mt-3 text-3xl font-bold tracking-[-0.04em] text-[color:var(--color-primary-dark)]">{value}</p>
      <p className="mt-2 text-sm leading-7 text-[color:var(--color-ink-soft)]">{hint}</p>
    </div>
  );
}
    ...
    <main>
            ...
            <div className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
              <div>
                ...
                <div className="mt-8 flex items-center gap-5">
                  <div className="relative h-20 w-20 overflow-hidden rounded-[1.6rem] border border-[color:var(--color-line)] bg-white shadow-[var(--shadow-card)]">
                  ...
                  <div>
                    ...
                    <h1 className="section-heading mt-3 text-[clamp(2.5rem,5vw,4.5rem)]">{profile.charity.name}</h1>
                  </div>
                </div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  {profile.charity.websiteUrl ? (
                    <a href={profile.charity.websiteUrl} target="_blank" rel="noreferrer" className="btn-primary">
              ...
              <aside className="surface-card p-7 sm:p-8">
                <h2 className="text-2xl font-bold tracking-[-0.03em] text-[color:var(--color-ink)]">Trust and profile summary</h2>
                <div className="mt-6 grid gap-4">
                  <div className="rounded-[1.4rem] border border-[color:var(--color-line)] bg-white p-5">
      ...
      <section className="section-shell-tight section-sandband">
        <div className="site-shell grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="surface-card p-7 sm:p-8">
            <h2 className="text-2xl font-bold tracking-[-0.03em] text-[color:var(--color-ink)]">Recent fundraiser pages</h2>
            ...
            <div className="mt-6 grid gap-4">
              {profile.fundraiserPages.length > 0 ? (
                profile.fundraiserPages.map((page) => (
                  <Link key={page.id} href={`/fundraise/${page.shortName}`} className="rounded-[1.4rem] border border-[color:var(--color-line)] bg-white p-5 transition-transform duration-200 hover:-translate-y-0.5">
                ...
                <div className="rounded-[1.4rem] border border-dashed border-[color:var(--color-line)] bg-white p-5 text-sm leading-7 text-[color:var(--color-ink-muted)]">
        ...
        <div className="site-shell">
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-3">
            {profile.charity.appeals.length > 0 ? (
              ...
              <div className="surface-card p-8 text-sm leading-7 text-[color:var(--color-ink-muted)]">
                This charity does not have any public active appeals right now.
              </div>
            )}
          </div>
        </div>
